        ...
        >
          ...
          >
            <template slot-scope="scope">
              <p v-if="userlist[scope.$index].idCard == 0">未发卡</p>
              <p v-if="userlist[scope.$index].idCard == 1">已发卡</p>
            </template>
          </el-table-column>
        </el-table>
      </template>
  ...
  data() {
    return {
      ...
      rulses: {
        ...
        code: [
          { required: true, message: "员工编号不能为空", trigger: "blur" },
          { min: 2, max: 3, message: "请输入2~3为数字" },
          {
            pattern: /^((?:[1-9]\d|(1\d\d)|(2([0-4]\d|5[0-5]))))$/,
            message: "员工编号格式不对",
            trigger: "blur",
          },
        ],
